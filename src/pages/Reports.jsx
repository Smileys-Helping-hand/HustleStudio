...
const Reports = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const salesQuery = query(collection(db, 'sales'), where('createdAt', '>=', fromDate), orderBy('createdAt', 'desc'));
  ...
  const exportPdf = useCallback(async () => {
    ...
    try {
      const storageRef = ref(storage, `reports/hustle-report-${period}-${Date.now()}.pdf`);
      await uploadBytes(storageRef, blob);
    } catch (error) {
      console.warn('[Reports] upload skipped', error);
    }
  }, [period, sales, summary, teamActivity.length]);
  ...
  const exportExcel = useCallback(async () => {
    ...
    try {
      const storageRef = ref(storage, `reports/hustle-report-${period}-${Date.now()}.xlsx`);
      await uploadBytes(storageRef, blob);
    } catch (error) {
